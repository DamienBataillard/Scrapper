...
def _log_request(source, url, status, cards, keyword=""):
...
def fetch_france_travail():
    ...
    try:
        for keyword in SEARCH_KEYWORDS[:2]:
            ...
            params = {"motsCles": keyword, "ville": SEARCH_LOCATION, "rayonRecherche": 50}
...
def fetch_linkedin():
    ...
    try:
        for keyword in SEARCH_KEYWORDS[:1]:
            ...
            params = {"keywords": keyword, "location": SEARCH_LOCATION, "sortBy": "DD", "f_TPR": "r604800"}
            r = requests.get(url, params=params, headers=HEADERS, timeout=15)
            soup = BeautifulSoup(r.text, "html.parser")
